(function ($) {
    ...
    $.fn.sakura = function (options) {
        ...
        var documentWidth = $(document).width(); // 화면 너비로 변경
        ...
        var petalCreator = function () {
            ...
            var startPosLeft = Math.random() * documentWidth - 100; // -100을 제거하여 화면 너비 내에서만 꽃잎이 생성되도록 조정
       ...
       // 화면 리사이즈 시 documentWidth 업데이트
        $(window).resize(function () {
            documentHeight = $(document).height();
            documentWidth = $(document).width(); // 화면 리사이즈에 따른 너비 업데이트를 window.innerWidth로 변경
        });
